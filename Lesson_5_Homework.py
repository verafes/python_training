...
# Task #1. Input a day of the week number (1-7) to the variable num_of_day. Using ternary operators, assign the value “Weekend” to the variable day if week number is 6 or 7, otherwise “Work day”. 
num_of_day = int(input("Please enter week number (1-7): "))
day = "Weekend" if num_of_day == 6 or num_of_day == 7 else "Work day" 
print(day)

print("\n---- Task 2. Lucky number")
# Task #2. Input a number to variable n. Using ternary operators, assign the value "Lucky number" to the variable is_lucky if the number is divisible by 9, otherwise "Not a lucky number". Print the result saying whether this number is lucky or not.
...
# 3. Create a variable "phrase" and assign the value "My eyes are green too!" to it. Using input command, ask the user, “What is your favorite color?” Replace the word “green” in phrase with the user's favorite color and print the result.
...
# 5. Create the variable s and assign any string value to it. Remove all vowels from the text. For example, s = “We like Python” --> “W lk Pythn”.
...
# 6. Using the input command, enter a string. Then enter the word to be replaced. Then enter the replacement word. Output the result. For example, entered: the statement "Have a good day", the word "good", the word "nice". The result should be: "Have a nice day"
...
# 7. Given a String. There are some spaces at the beginning and end of the statement. Get a string where each word is capitalized, spaces at the beginning of the statement are removed, and spaces at the end of the statement are replaced with the same number of exclamation marks. For example, s = "    I like summer   ". Get "s = "I Like Summer!!!"
